...
int pua_json_update_presentity(str* event, str* realm, str* user, str* etag, str* sender, str* body, int expires, int reset) {
	int ret;
	if(!event->len)
	{
	...
	ret = presence_api.update_presentity(event, realm, user, etag, sender, body, expires, reset);
	return ret;
}

int pua_json_publish_presence_to_presentity(
		struct json_object *json_obj, int refresh)
{
	int ret = 1;
	str from = { 0, 0 };
	...
	pua_json_update_presentity(&event, &from_realm, &from_user, &callid, &from, &presence_body, expires, 1);

	if(refresh) {
		LM_ERR("PERFORM REFReSH NOW\n");
		//if(presence_api.pres_refresh_watchers_db_notify(&from, &event) < 0) {
		//	LM_ERR("sending pres_refresh_watchers\n");
		//}
	}
...
int pua_json_publish_mwi_to_presentity(
		struct json_object *json_obj, int refresh)
{
	int ret = 1;
	...
	pua_json_update_presentity(&event, &from_realm, &from_user, &callid, &from, &mwi_body, expires, 1);
...
int pua_json_publish_dialoginfo_to_presentity(struct json_object *json_obj, int refresh) {
	int ret = 1;
	...
	ExpiresObj = json_api.get_object(json_obj, "Flush-Level");
	if (ExpiresObj != NULL) {
	...
	pua_json_update_presentity(&event, &pres_realm, &pres_user, &callid, &sender, &dialoginfo_body, expires, reset);
...
int pua_json_publish(struct sip_msg* msg, char *json, int refresh) {
	...
	if (event_name.len == 6 && strncmp(event_name.s, "update", 6) == 0) {
		...
		if (event_package.len == str_event_dialog.len
			...
			ret = pua_json_publish_mwi_to_presentity(json_obj, refresh);
