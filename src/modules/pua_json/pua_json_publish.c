...
int pua_json_update_presentity(str* event, str* realm, str* user, str* etag, str* sender, str* body, int expires) {
	int ret;
	if(!event->len)
	{
	...
	ret = presence_api.update_presentity(event, realm, user, etag, sender, body, expires, 1);
	return ret;
}
		...
		struct json_object *json_obj, int refresh)
{
	...
	pua_json_update_presentity(&event, &from_realm, &from_user, &callid, &from, &presence_body, expires);

	if(refresh) {
		if(presence_api.pres_refresh_watchers(&from, &event, 1) < 0) {
		...
		struct json_object *json_obj, int refresh)
{
	...
	pua_json_update_presentity(&event, &from_realm, &from_user, &callid, &from, &mwi_body, expires);
...
int pua_json_publish_dialoginfo_to_presentity(struct json_object *json_obj, int refresh) {
	...
	pua_json_update_presentity(&event, &pres_realm, &pres_user, &callid, &sender, &dialoginfo_body, expires);
